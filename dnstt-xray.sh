...
show_spinner() {
    ...
    while kill -0 "$pid" 2>/dev/null; do
        printf "\r${C_BLUE}${frames[$((frame % 10))]}${C_RESET} %s" "$message"
        ((frame++))
        sleep 0.1
    done
...
check_and_free_ports() {
    ...
    if ss -H -lunp "( sport = :$port )" 2>/dev/null | grep -q .; then
        ...
        conflicting_pid=$(ss -H -lunp "( sport = :$port )" 2>/dev/null | grep -oP 'pid=\K[0-9]+' | head -n1)
        echo -e "${C_YELLOW}⚠️ Puerto $port en uso (PID: ${conflicting_pid:-N/A}).${C_RESET}"
        read -r -p "👉 ¿Liberar este puerto? (y/n): " kill_confirm
...
check_and_open_firewall_port() {
    ...
    if command -v ufw &>/dev/null && ufw status 2>/dev/null | grep -q "Status: active"; then
        ...
        if ! ufw status 2>/dev/null | grep -qw "$port/$protocol"; then
            echo -e "${C_YELLOW}🔥 UFW activo. Puerto ${port}/${protocol} cerrado.${C_RESET}"
            read -r -p "👉 ¿Abrir este puerto? (y/n): " confirm
            ...
            echo -e "${C_GREEN}✅ Puerto ${port}/${protocol} ya abierto en UFW.${C_RESET}"
        fi
    fi
    ...
    if command -v firewall-cmd &>/dev/null && systemctl is-active --quiet firewalld; then
        ...
        if ! firewall-cmd --list-ports --permanent 2>/dev/null | grep -qw "$port/$protocol"; then
            echo -e "${C_YELLOW}🔥 firewalld activo. Puerto ${port}/${protocol} no abierto.${C_RESET}"
            read -r -p "👉 ¿Abrir este puerto? (y/n): " confirm
            if [[ "$confirm" == "y" || "$confirm" == "Y" ]]; then
            ...
            echo -e "${C_GREEN}✅ Puerto ${port}/${protocol} ya abierto en firewalld.${C_RESET}"
        fi
    fi
...
show_dnstt_details() {
    if [ -f "$DNSTT_CONFIG_FILE" ]; then
        set +u
        source "$DNSTT_CONFIG_FILE" 2>/dev/null || true
        set -u
        echo -e "\n${C_GREEN}=====================================================${C_RESET}"
...
install_dnstt() {
    ...
    if ss -lunp 2>/dev/null | grep -q ':53 '; then
        ...
        read -r -p "👉 ¿Permitir que el script lo libere? (y/n): " resolve_confirm
    ...
    read -r -p "👉 Selecciona (1/2) [2]: " fwd_choice
    ...
    while [[ -z "$NS_DOMAIN" ]]; do
        read -r -p "👉 Dominio del Nameserver (ej: ns1.tudominio.com): " NS_DOMAIN
        if [[ -z "$NS_DOMAIN" ]]; then
    ...
    while [[ -z "$TUNNEL_DOMAIN" ]]; do
        read -r -p "👉 Dominio del Túnel (ej: tun.tudominio.com): " TUNNEL_DOMAIN
    ...
    read -r -p "👉 Valor MTU (ej: 512, 1200) o presiona Enter para default: " mtu_value
    local mtu_string=""
    ...
    echo -e "${C_BLUE}ℹ️ Detectado: $arch.${C_RESET}"
    ...
    if [ $download_status -ne 0 ] || [ ! -f "$DNSTT_BINARY" ]; then
...
Type=simple
User=root
ExecStart=__DNSTT_BINARY__ -udp :53__MTU_STRING__ -privkey-file __KEY_DIR__/server.key __TUNNEL_DOMAIN__ __FORWARD_TARGET__
Restart=always
RestartSec=3
[Install]
WantedBy=multi-user.target
EOF
    sed -i "s|__DNSTT_BINARY__|$DNSTT_BINARY|g" "$DNSTT_SERVICE_FILE"
    sed -i "s|__MTU_STRING__|$mtu_string|g" "$DNSTT_SERVICE_FILE"
    sed -i "s|__KEY_DIR__|$DNSTT_KEYS_DIR|g" "$DNSTT_SERVICE_FILE"
    ...
    systemctl start dnstt.service 2>/dev/null || true
    sleep 2
...
uninstall_dnstt() {
    ...
    if [ -f "$DNSTT_CONFIG_FILE" ]; then
        ...
        echo -e "   • NS: ${C_YELLOW}${NS_DOMAIN:-No definido}${C_RESET}"
        echo -e "   • Túnel: ${C_YELLOW}${TUNNEL_DOMAIN:-No definido}${C_RESET}"
    fi
